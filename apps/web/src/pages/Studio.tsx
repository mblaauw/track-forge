...
function parseLyricsValue(value: string): {
  ...
  try {
    ...
    if (parsed.document?.sections) {
      ...
      const sections = parsed.document.sections.map(
        (s: { type: string; lines?: string[]; label?: string }) => {
          const lines = s.lines || [];
  ...
  for (const line of lines) totalSyllables += syllableCount(line);
  return {
    sections: lines.length > 0 ? [{ type: "text", lines }] : [],
    totalSyllables,
  };
}
...
export function Studio({ id }: { id: string }) {
  ...
  useEffect(() => {
    if (!actualId) return;
    setLoading(true);
    Promise.all([fetchJob(actualId), fetchVersions(actualId)])
      .then(([j, vs]) => {
  ...
  useEffect(() => {
    const pending = generations.filter(
      (g) => !g.audioUrl && g.status !== "completed" && g.status !== "failed",
    );
  ...
  const styleArtifact = selectedVersion?.artifacts?.find(
    (a) => a.type === "style",
  );
  const lyricsArtifact = selectedVersion?.artifacts?.find(
    (a) => a.type === "lyrics",
  );

  const styleText = styleArtifact?.value || "";
  const parsedLyrics = lyricsArtifact?.value
    ? parseLyricsValue(lyricsArtifact.value)
    : { sections: [], totalSyllables: 0 };
  ...
  const takesCount = visibleGenerations.filter(
  ...
  const toggleFavored = async (genId: string) => {
    try {
      ...
      setGenerations((prev) =>
        prev.map((pg) =>
          pg.id === genId ? { ...pg, isFavorite: g.isFavorite } : pg,
        ),
      );
    } catch {
      ...
      <div style="padding:24px;display:flex;flex-direction:column;gap:24px;">
        <div class="bundle-header">
          <div
            ...
            style={{ height: 24, width: 120, borderRadius: 4 }}
          />
          <div
            class="skeleton-block"
            style={{ height: 32, width: 200, borderRadius: 4 }}
          />
          <div
            class="skeleton-block"
            style={{ height: 24, width: 120, borderRadius: 4 }}
    ...
    <div>
      ...
      {versions.length > 0 && (
        <div class="studio-layout">
          ...
          <div class="artifact-panel">
            <div class="artifact-panel-header">
              ◆ Lyric Sheet
              {parsedLyrics.totalSyllables > 0
                ? ` (${parsedLyrics.totalSyllables} syllables)`
                : ""}
            </div>
            <div class="artifact-panel-body lyric-sheet">
              {parsedLyrics.sections.length > 0 ? (
                <>
                  ...
                  <div class="total-syllables">
                    Total: {parsedLyrics.totalSyllables} syllables
                  </div>
                </>
              ) : (
                <div style="color:var(--text-dim);">&mdash;</div>
      ...
      <div class="takes-section">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
          <h2 class="takes-title" style="margin:0;">
            ◆ Audio Takes
          </h2>
          ...
          <div class="takes-list">
            {visibleGenerations.map((gen, i) => {
              ...
              return (
                <div
                  key={gen.id}
                  class={`take-card${isFavored ? " favored" : ""}`}
                >
                  <button
                  ...
                  <div class="take-info">
                    <div class="take-title">
                      {gen.generatedTitle || `Take ${i + 1}`}
                    </div>
                    <div class="take-meta">
                      {ready ? gen.style || "" : "Rendering…"}
                    </div>
                  </div>
                    ...
                    {Array.from({ length: 46 }, (_, j) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            height: `${(hashString(gen.id + j, 3, 37) % 34) + 3}px`,
                          }}
                        />
                      );
                    })}
                  ...
                  {gen.duration != null && (
                    <span class="take-duration">
                      {fmtDuration(gen.duration)}
                    </span>
                  )}
