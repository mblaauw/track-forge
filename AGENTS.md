...
| `packages/genre-*`      | `@track-forge/genre-*`      | Genre modules: edm, hiphop, ambient                                                                          |
| `packages/test-support` | `@track-forge/test-support` | Shared test helpers                                                                                          |

## Backend architecture

**Routes** (`apps/server/src/routes/*`): jobs, versions, health, suno, events, import-export, preview-style. Routed via route-type name on `server.get/post/patch/delete`.
...
Stage state persisted as JSON in `job.stageData`.
...
Presets, tag categories, song structures, taxonomy, descriptor categories + defaults, lyric themes, section functions, delta palette, section palette, vocal presets live in `config/genres/*.yaml` — no DB.
...
**SSE**: `connectJobEvents(id, handlers)` in `api.ts`. Drives the 8-bar forge strip (3 pipeline stages + 1 Suno render via synthetic `suno_render` events). History replay on reconnect.
...
Implement `GenreModule` from `@track-forge/genre-core`:
...
The `inputSchema` validates session inputs for the genre. `defaults` provides initial values. The optional fields augment the genre at runtime from `config/genres/*.yaml`.

**lyricsMode** must be included in `inputSchema` (`strict_instrumental`, `guided_instrumental`, `full_lyrics`).

## LLM behavior

Kimi k2.5 recommended (~13s/call). DeepSeek v4 flash uses 80-92% tokens on hidden `reasoning_content` — need `max_tokens` ≥ 8192 to see visible output. Per-stage max_tokens tuned for kimi: lyrics 2048.
...
- **`lyricsMode === "strict_instrumental"` skips LLM lyrics call** — handled in `handleLyricsWriting`.
- **Cascade delete order** (from jobs.ts): sunoTracks → generations → versions → jobEvents → jobs.
...
- **Descriptor/vocab data is YAML-served** — `GET /api/genres/:id/descriptor-defaults` returns `{ categories, defaults, lyricThemes, sectionFunctions, deltaPalette, sectionPalette, vocalPresets }`. The UI no longer has hardcoded `LYRIC_THEMES`, `DELTA_PALETTE`, etc. — test against the API, not local constants.
- **Vocal delivery editor visibility** — gated solely by `sectionIsVocal(sec)`. It checks `deltas` for `"instrumental"` (hides), `"vocal focus"`/`"catchy"` (shows), or section name matching `/verse|chorus|hook|pre-chorus|refrain|bridge|drop/` (shows).
