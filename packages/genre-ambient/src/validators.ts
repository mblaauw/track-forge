...
export const AMBIENT_VALIDATORS: GenreValidators<AmbientInputs> = {
  input: (inputs) => {
    ...
    if (inputs.bpm < 40 || inputs.bpm > 120) {
      errors.push({ field: "bpm", message: "BPM must be between 40 and 120 for ambient" });
    }
    ...
    if (!inputs.soundscape) {
      errors.push({ field: "soundscape", message: "Soundscape description is required" });
    }
  ...
  blueprint: (data) => {
    ...
    if (bp.bpm < 40 || bp.bpm > 120) {
      errors.push({ field: "bpm", message: `BPM ${bp.bpm} out of ambient range` });
    }
